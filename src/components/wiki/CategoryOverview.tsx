...
export function CategoryOverview({ categoryPath }: CategoryOverviewProps) {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-grass-dark mb-2">{categoryLabel}</h1>
      {categoryMeta && (
        <p className="text-gray-600 text-lg mb-8">{categoryMeta.description}</p>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {articles.map((article) => (
            <Link
              ...
              className="block p-5 rounded-lg border border-gray-200 hover:border-grass/50 hover:shadow-md transition-all"
            >
